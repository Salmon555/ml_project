...
class Mainwindow(QWidget):
    ...
    def show_status(self):
        ...
        if output[1] != "":
            ...
            if machine_sequence1 == machine_sequence2:
                machine = data_processing_functions.machine_sequence_to_machine_name(machine_sequence1,int(machine_size_type[0]),int(machine_size_type[1]))
                machine += "\n" + "Corresponding sequence:" +"\n" +data_processing_functions.status_to_usefulstatus(output[0])
            else:
                machine = data_processing_functions.machine_sequence_to_machine_name(machine_sequence1,int(machine_size_type[0]),int(machine_size_type[1]))
                machine += "\n"
                machine += data_processing_functions.machine_sequence_to_machine_name(machine_sequence2,int(machine_size_type[0]),int(machine_size_type[1]))
                machine += "\n" + "Corresponding sequence:" +"\n" +data_processing_functions.status_to_usefulstatus(output[0])
                machine += "\n" + data_processing_functions.status_to_usefulstatus(output[1])
        else:
            machine_sequence1 = id_processing.machine_type(output[0])[:-1]
            machine = data_processing_functions.machine_sequence_to_machine_name(machine_sequence1,machine_size_type[0],machine_size_type[1])
            machine += "\n" + data_processing_functions.status_to_usefulstatus(output[0])
    ...
    def processing_id(self, id) -> str:   # processing product_id
